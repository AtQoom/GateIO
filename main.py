...
def update_all_position_states():
    with position_lock:
        ...
        if all_positions is None: return
        active_positions_set = set()
        for pos in all_positions:
            symbol, side = normalize_symbol(pos.contract), 'long' if pos.mode == 'dual_long' else 'short'
            if symbol not in position_state: initialize_states()
            cfg = get_symbol_config(symbol)
            state = position_state[symbol][side]
            state.update({"price": Decimal(str(pos.entry_price)), "size": Decimal(str(pos.size)), "value": Decimal(str(pos.size)) * Decimal(str(pos.mark_price)) * cfg["contract_size"]})
            if state["entry_count"] == 0 and state["size"] > 0:
                log_debug(f"🔄 수동 포지션 감지", f"{symbol} {side.upper()}")
        ...
        for symbol, sides in position_state.items():
            for side in ["long", "short"]:
                if (symbol, side) not in active_positions_set and sides[side]["size"] > 0:
                    log_debug(f"🔄 수동 청산 감지 ({symbol}_{side.upper()})", f"서버: {sides[side]['size']}, API: 없음")
