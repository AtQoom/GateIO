...
SETTLE = "usdt"#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
def on_entry_signal_received(symbol_raw, side_raw):
    ...
    if equity > 0 and price > 0:
        ...
        if pos and pos["avg_entry_price"] > 0:
            ...
            tp_abs = entry_price * (Decimal("1.0") + tp_rate) if side == "buy" else entry_price * (Decimal("1.0") - tp_rate)
            sl_abs = entry_price * (Decimal("1.0") - sl_rate) if side == "buy" else entry_price * (Decimal("1.0") + sl_rate)
            store_tp_sl(symbol, float(tp_abs), float(sl_abs), entry_count + 1)
            log_debug("TP/SL지정", f"{symbol} 진입가: {entry_price}, TP: {tp_abs}, SL: {sl_abs}")
    else:
